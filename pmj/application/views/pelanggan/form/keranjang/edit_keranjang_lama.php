<ul class="breadcrumb">
		...
		<li class="active"> Edit Keranjang </li>
    </ul>
				  ...
				  <div class="control-group">
					...
					<?php }else{ ?>
						...
						<tr>
							...
							<td>:</td>
							<td>
								<input type="hidden" name="id_pesanan" class="form-control span1" required value="<?php echo $id_pesanan ?>">
							...
							<td>
								<select name="warna" class="form-control">
									<?php 	foreach ($warna as $k => $v) { ?>
										<option value="<?php echo $v['warna'] ?>" <?php if($pesanan['id_warna']==$v['id_warna']){echo "selected";} ?>><?php echo $v['warna'] ?></option>
									<?php } ?>
								</select>
							</td>
						</tr>
							...
							<td>
								<table class="table">
									...
									<?php foreach ($topping as $k => $v) { 
										...
										$topping_dipesan = $this->db->query("SELECT * from topping_pesanan where id_topping='$id_topping' and id_pesanan='$id_pesanan'")->row_array();
										?>
									<tr>
										<td><?php echo $v['nama_topping'] ?></td>	
										<td><?php echo number_format($v['harga']) ?></td>	
										<td>
											<input type="hidden" name="id_topping[]" class="span1" value="<?php echo $v['id_topping'] ?>">
											<input type="number" name="qty_topping[]" class="span1" value="<?php echo $topping_dipesan['banyak_topping'] ?>">
										</td>	
									</tr>
									<?php } ?>
								</table>
